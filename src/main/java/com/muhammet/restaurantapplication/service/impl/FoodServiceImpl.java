...
public class FoodServiceImpl implements FoodService {
    ...
    public FoodServiceImpl(FoodRepository repository, BranchRepository branchRepository, BranchService branchService, ModelMapper modelMapper) {
    ...
    public List<GetAllFoodsResponse> getAllFoods() {
        List<Food> foods = repository.findAll();

        var getAllFoodsResponses=foods.
                stream()
                .map(food -> this.modelMapper.map(food, GetAllFoodsResponse.class)).collect(Collectors.toList());
    ...
    public void updateFood(UpdateFoodRequest updateFoodRequest){
        ...
        var updating=getFoodById(updateFoodRequest.getId());
        //  var branch=branchService.getById(food.getBranch().getId());
        updating.setFoodName(updateFoodRequest.getFoodName());
        updating.setPrice(updateFoodRequest.getPrice());
    ...
    public Food getFoodById(Long id){
        return repository.findById(id).orElseThrow(()->new FoodNotFoundException("Böyle bir yemek bulunamadı"));
    }
